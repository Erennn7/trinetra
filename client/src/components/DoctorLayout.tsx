...
const NAV_ITEMS = [
  ...
  { label: 'My Profile', icon: <Person sx={{ fontSize: 18 }} />, path: 'profile' },
];
...
export default function DoctorLayout() {
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(q, (snap) => setQueueCount(snap.size), (err) => console.error('DoctorLayout queue:', err));
    return unsub;
  }, []);
      ...
      <aside style={{
        ...
        <div style={{ padding: '12px 16px', borderTop: '1px solid rgba(255,255,255,0.06)' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '6px 10px', background: 'rgba(196,181,253,0.06)', border: '1px solid rgba(196,181,253,0.15)', borderRadius: 8 }}>
            ...
            <div>
              <p style={{ color: '#fff', fontSize: '0.75rem', fontWeight: 600, margin: 0 }}>{profile?.name || 'Doctor'}</p>
              <p style={{ color: 'rgba(255,255,255,0.35)', fontSize: '0.6rem', margin: 0 }}>Doctor</p>
            </div>
          </div>
        </div>
      </aside>
