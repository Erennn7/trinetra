...
class Config:
    ...
    # Pandharpur Location — default city
    MAHAKUMBH_LAT = float(os.getenv('MAHAKUMBH_LAT', '17.6784'))
    MAHAKUMBH_LON = float(os.getenv('MAHAKUMBH_LON', '75.3294'))

    DEFAULT_CITY = os.getenv('DEFAULT_CITY', 'pandharpur')
    ...
    CITIES = {
        'pandharpur': {
            'id': 'pandharpur',
            'name': 'Pandharpur',
            'display_name': 'Pandharpur (Wari)',
            'event': 'Wari Pilgrimage',
            'lat': 17.6784,
            'lon': 75.3294,
            'zoom': 15,
            'is_primary': True,
            'description': 'Sacred Vitthal temple town on Bhima river — Wari pilgrimage hub'
        }
    }
